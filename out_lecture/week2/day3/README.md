...
- 주말 챌린지에서는 3-tier 구조를 직접 구성하고, frontend->backend, backend->database만 허용되는 network evidence를 남긴다.
- image 최적화는 감으로 고르지 않고 최초 build 시간과 image size 숫자로 비교한다.
...
- 9세션 챌린지: 3-tier image delivery - frontend/backend/database network, volume, log evidence 남기기
...
| `session-09-challenge.md` | 3-tier network, volume, log, troubleshooting 주말 챌린지 설명 |
| `labs/weekend-3tier-challenge/README.md` | 주말 챌린지 lab 파일 안내 |
| `labs/weekend-3tier-challenge/SUBMISSION.md` | 주말 챌린지 제출 표 |
...
## Weekend Challenge
Day 3 주말 챌린지는 단일 static site를 넘어 3-tier 구조까지 확장한다. Frontend는 `index.html`과 `styles.css`를 제공하고, backend는 Node.js hello world API만 띄워도 된다. Database는 `postgres:16`을 host port로 열지 않고 named volume으로 data lifecycle을 확인한다.

진행 문서는 `session-09-challenge.md`로 분리한다. 제출물은 `labs/weekend-3tier-challenge/SUBMISSION.md`에 표로 남긴다. 필수 증거는 architecture 설명, network 성공/차단 결과, volume 유지 결과, service logs, troubleshooting RCA, base image별 최초 build 시간과 image size다.
...
7. 주말 챌린지를 선택했다면 3-tier network, volume, log, image size/build time evidence를 제출 표에 남겼다.
```
