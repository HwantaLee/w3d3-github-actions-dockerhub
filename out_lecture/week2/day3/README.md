...
- `docker history`, `docker image inspect`, image size, cache hit/miss를 evidence로 남긴다.
...
- missing file, wrong CMD, wrong port, bloated context 같은 build/runtime 실패를 RCA로 기록한다.
...
| `assets/lesson-05-build-troubleshooting.png` | build cache/layer failure |
| `assets/lesson-06-image-registry-provenance.png` | registry와 image provenance |
| `assets/lesson-07-tag-push-pull-gate.png` | tag/push/pull security gate |
| `assets/lesson-08-image-build-rca.png` | image build failure RCA |

## Linux Preflight Test Evidence
Day 3 build/run 명령은 Linux 또는 macOS Docker 환경에서 사전 테스트한다. 핵심은 exact image ID가 아니라 build 성공, HTTP 확인, history/inspect evidence, cleanup이다.
...
| Build result | image 생성 성공, build context size 기록 |
| Run command | `docker run -d --name paperclip-day3-static -p 18083:80 paperclip-static-site:day3` |
| HTTP evidence | `curl -I http://localhost:18083` -> `HTTP/1.1 200 OK` pattern |
| History evidence | `docker history paperclip-static-site:day3`에서 base/COPY layer 확인 |
| Inspect evidence | image ID, repo tag, architecture, size 확인 |
| Cache evidence | source 변경 전후 cache hit/miss 기록 |
| Failure drill | missing file, wrong CMD, wrong port, bloated context 중 하나 재현 |

## Required Evidence
| Evidence | 제출 기준 |
|---|---|
| Dockerfile evidence | instruction별 역할 설명 |
| Build evidence | build command, context size, image tag |
| Run evidence | container ID, `docker ps`, HTTP status |
| History/inspect evidence | layer, image ID, size, architecture |
| Cache evidence | cache hit/miss 또는 source 변경 rebuild 결과 |
| Registry evidence | official image 기준, tag/digest 또는 push gate |
| Failure evidence | missing file, wrong CMD, wrong port, bloated context 중 하나 RCA |
| Cleanup evidence | container/image 정리 결과 |

## End-Of-Day Checklist
- [ ] Dockerfile instruction 역할을 설명했다.
- [ ] `docker build`로 표준 앱 image를 만들었다.
- [ ] `docker run`과 `curl`로 HTTP 200을 확인했다.
- [ ] `docker history`와 `docker image inspect`를 기록했다.
- [ ] build cache 또는 source 변경 rebuild를 확인했다.
- [ ] official image tag/digest 또는 registry 출처를 확인했다.
- [ ] image build/run failure drill을 RCA 형식으로 기록했다.
