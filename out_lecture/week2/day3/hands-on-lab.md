...
## Weekend Challenge: 3-tier runtime evidence
Day 3 기본 lab을 끝낸 뒤에는 `session-09-challenge.md`를 기준으로 주말 챌린지를 진행한다.

```bash
cd /mnt/d/paperclip/week2/day3
sed -n '1,260p' session-09-challenge.md
sed -n '1,180p' labs/weekend-3tier-challenge/SUBMISSION.md
```

목표는 frontend, backend, database를 각각 다른 책임으로 실행하고 다음 증거를 제출 표에 남기는 것이다. Frontend는 `index.html`과 `styles.css`, backend는 Node.js hello world, database는 PostgreSQL 16 + named volume이면 충분하다.

| Evidence | 최소 통과 기준 |
|---|---|
| Architecture | frontend, backend, database container와 network 역할을 표로 설명한다. |
| Network | frontend -> backend 성공, backend -> db 성공, frontend -> db 실패를 command output으로 남긴다. |
| Volume | `paperclip-weekend-pgdata` named volume과 재시작 후 `Skipping initialization` 신호를 확인한다. |
| Logs | frontend/backend/db 각각 `docker logs --tail` 핵심 line을 남긴다. |
| Troubleshooting | 502, DB 연결 실패, stale volume, build bloating 중 하나를 RCA 형식으로 정리한다. |
| Optimization | frontend/backend base image별 최초 build seconds와 image size를 표로 비교한다. |
| Tag or upload | push는 선택이지만 tag 또는 registry URL을 남긴다. |

이 챌린지는 Docker Compose 없이 일부러 `docker network`, `docker run`, `docker exec`, `docker logs`, named volume을 직접 사용한다. 다음 주 Kubernetes와 AWS/Terraform에서는 같은 구조를 더 선언적인 방식으로 다시 표현한다.
