...
| Docker inspect | container metadata, network, env, mount evidence 확인 |
...
| SRE incident practice | 실패 재현, fix, recheck, prevention 기록 |
...
Observability는 Kubernetes부터 시작하는 주제가 아니다. 로컬 Docker에서도 stdout/stderr, process list, metadata, resource usage를 확인하는 습관이 있어야 한다. 학생은 실패를 보면 먼저 logs, inspect, port, env, process 중 어디에서 evidence를 얻을지 판단한다.
...
| SRE/DevOps evidence | logs, inspect, exec, stats, curl 결과를 함께 남김 |

## Completion Evidence

학생은 Day 4 종료 시점에 다음을 제출할 수 있어야 한다.
...
- 의도적 실패를 재현하고 fix/recheck/prevention을 기록한 RCA
