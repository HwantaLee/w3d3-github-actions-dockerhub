...
```text
container가 정상인지, 왜 실패했는지, 무엇을 지워도 되는지는 어떤 명령과 evidence로 판단하는가?
```

Day 4는 Compose 전 마지막 `docker run` 심화일이다. Compose로 넘어가기 전에 `-e`, `--env-file`, `logs`, `inspect`, `exec`, `stats`, restart policy, cleanup audit을 익힌다. 실습은 성공만 확인하지 않고 missing env, wrong port, wrong network, stale volume, bad image tag를 일부러 만들어 RCA로 정리한다.
...
| Evidence | 제출 기준 |
|---|---|
| Env evidence | `-e` 또는 `--env-file`로 주입한 config와 secret masking |
| Log evidence | 정상 startup log 또는 failure log 핵심 줄 |
| Inspect evidence | env/network/mount/port 중 2개 이상 |
| Exec evidence | container 내부 command 결과 |
| Stats/restart evidence | resource 관찰 또는 restart policy 결과 |
| Failure evidence | missing env, wrong port, wrong network, stale volume, bad image tag 중 하나 RCA |
| Cleanup evidence | container/image/network/volume 정리 결과와 data 보존 판단 |
| Compose mapping evidence | 긴 `docker run` 옵션을 compose 항목으로 옮긴 표 |
...
- [ ] missing env, wrong port, wrong network, stale volume, bad image tag 중 하나를 RCA로 기록했다.
...
| 이전 학습 | Day 4 runtime 확인 | evidence |
|---|---|---|
...
| CS2023 Skill | run/logs/inspect/exec/stats 수행 | command evidence |
...
| Check | 정상 상태는 무엇인가 | HTTP/SQL/log/status evidence |
...
- [ ] 학생이 정상과 장애를 서로 다른 evidence로 확인한다.
...
2. env/config 주입 방식과 secret masking 기준이 기록되어 있다.
3. logs/inspect/exec/stats 중 필요한 evidence를 선별해 정상/장애를 판단했다.
4. missing env, wrong port, wrong network, stale volume, bad image tag 중 하나의 failure drill이 있다.
5. container/image/network/volume cleanup 명령과 data 삭제 위험이 README에 기록되어 있다.
