...
- Linux 장비를 사용하는 학생은 Docker Engine 설치 경로를 기본으로 따른다. Desktop for Linux를 이미 쓰거나 조직 정책상 필요하면 예외 경로로 표시하고, Desktop의 자체 Linux VM/context와 host Engine daemon의 차이를 기록한다.
...
| Docker runtime status | macOS Desktop running 또는 Linux Engine daemon 상태/blocker 기록 |
...
- [ ] macOS Docker Desktop 또는 Linux Docker Engine 설치/blocker를 기록했다.
