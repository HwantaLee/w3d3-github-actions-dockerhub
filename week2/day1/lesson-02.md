...
- 기본 실습 환경은 macOS Docker Desktop과 Linux Docker Desktop/Engine 경로를 나누어 안내한다.
- Docker Hub 로그인과 credential 보호 기준을 이해한다.
- 설치 실패를 OS 종류, Mac chip, Linux 배포판, 권한, daemon 상태, error message 기준으로 기록한다.
...
이 교시는 설치 성공을 경쟁시키는 시간이 아니라, 실행 기반을 증거로 분리하는 시간이다. Docker 설치는 OS와 장비 정책의 영향을 많이 받는다. 같은 "Mac"이라고 해도 Apple silicon과 Intel Mac의 조건이 다를 수 있고, 같은 "Linux"라고 해도 Docker Desktop for Linux를 쓰는지 Docker Engine을 쓰는지에 따라 확인해야 할 대상이 달라진다.
...
설치 실패자는 수업에서 제외하지 않는다. 설치 실패도 OS, chip, 배포판, 권한, daemon 상태, error message를 기준으로 정리하면 유효한 운영 evidence가 된다. 이 방식은 뒤에서 container 실행 실패를 분석할 때 그대로 반복된다. 즉, 설치 수업은 Docker 개념 수업이기도 하다. 학생들은 "실패를 기록하는 방식"을 여기서 배운다.
...
Linux 장비는 두 갈래다. Docker Desktop for Linux는 자체 Linux VM을 만들고 전용 Docker context를 사용한다. Docker Engine은 host Linux daemon을 직접 사용한다. 수업에서는 둘 중 하나를 선택해 `docker version`, `docker compose version`, `docker run --rm hello-world`까지 확인한다.
...
![Docker Desktop 설치 점검 triage](./assets/lesson-02-docker-desktop-triage.png)
...
```mermaid
flowchart TD
    A["OS별 Docker 실행 기반 준비됐는가?"] -->|macOS Desktop yes| B["docker version 실행"]
    A -->|Linux Desktop/Engine yes| B
    A -->|no| C["macOS 권한 / Linux daemon / Windows WSL 2 확인"]
    B --> D{"server 응답이 있는가?"}
    D -->|yes| E["hello-world 준비"]
    D -->|no| F["Docker daemon / Desktop running 상태 확인"]
    C --> G["공식 문서 URL과 error message 기록"]
    F --> G
```
...
Linux 수강생은 먼저 Docker Desktop for Linux를 쓸지 Docker Engine을 쓸지 결정한다. Docker Desktop for Linux 공식 문서는 Desktop이 자체 Linux VM과 Docker context를 만든다고 설명하고, 이미 host에서 Docker Engine이 실행 중이면 Desktop과 동시에 같은 port를 publish할 때 충돌이 날 수 있다고 안내한다. Docker Engine Ubuntu 경로를 선택하면 apt repository 설정과 daemon 실행 상태를 확인한다.
...
| Linux 경로 | Docker Desktop for Linux 또는 Docker Engine | Desktop VM/context와 host daemon 차이 |
...
3. Linux는 Docker Desktop for Linux 또는 Docker Engine 중 선택한 경로의 설치 상태를 확인한다.
4. Desktop 또는 daemon이 running 상태가 될 때까지 기다린다.
...
설치가 막힌 학생은 아래 형식으로 blocker를 정리한다. "안 됩니다"가 아니라 어느 단계에서 어떤 증상이 있었는지를 남겨야 후반 PostgreSQL 실습에서 어떤 대체 경로를 쓸지 결정할 수 있다.
...
- Docker Docs: Install Docker Desktop on Linux, https://docs.docker.com/desktop/setup/install/linux/ - Linux Desktop의 VM/context와 host Engine 차이를 확인하는 기준이다.
