...
## 처음이면 여기부터
Week 2는 Docker가 반드시 필요하다. macOS는 Docker Desktop을 기본 경로로 진행하고, Linux는 Docker Engine 설치를 기본 경로로 진행한다. Docker Desktop for Linux는 조직 장비 정책상 필요한 경우의 예외 경로로만 분리해 기록한다. 설치가 끝난 뒤에는 "앱을 열었다"가 아니라 CLI evidence로 확인한다.
