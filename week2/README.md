...
강의 본문에는 분 단위 흐름표를 넣지 않는다. 시간 배분은 강사용 참고로만 사용하고, 학생용 자료는 개념 전개, 실습 명령, expected evidence, 흔한 오해, cleanup 기준을 중심으로 작성한다.
...
- PostgreSQL SQL 조작 evidence 1개: user, database, table, insert, query
- storage/network evidence 2개 이상
- image/Dockerfile/registry evidence 2개 이상
- env/logs/inspect/exec failure drill evidence 1개 이상
- Compose architecture 실행 evidence 2개 이상
- port, log, environment variable, volume, network 중 하나 이상을 포함한 장애 분석 기록 1개
- 3주차 학습 전 Docker readiness checklist

## Assessments
- [Week 2 객관식 문제 세트](./assessments/week2-multiple-choice-questions.md)

## Practice Environment
| 항목 | 기준 |
|---|---|
| OS | macOS와 Linux를 기본 경로로 분리. Windows 사용자는 WSL 2/가상화/권한 예외 경로를 별도 기록 |
| Docker | macOS는 Docker Desktop, Linux는 Docker Desktop 또는 Docker Engine 중 하나를 공식 문서 기준으로 설치/확인 |
...
| Linux Desktop install | https://docs.docker.com/desktop/setup/install/linux/ | Linux VM, Docker context, host Engine 차이 |
...
- [ ] PostgreSQL container에서 user/database/table/row를 만들고 query 결과를 기록했다.
...
- [ ] official image tag/digest 또는 `docker image inspect` 결과를 기록했다.
- [ ] env/config 주입, logs/inspect/exec, secret 비노출 기준을 기록했다.
