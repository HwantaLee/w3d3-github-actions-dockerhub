...
```mermaid
flowchart TD
    A[missing 항목 선택] --> B{실행이 깨졌는가?}
    B -- 예 --> C[경로, 서버, URL 먼저 수정]
    B -- 아니오 --> D{Evidence가 부족한가?}
    D -- 예 --> E[확인 기록과 README 보완]
    D -- 아니오 --> F[작은 UI/error state만 수정]
    C --> G[Fresh start 재검증]
    E --> G
    F --> G
```
...
### 다음 연결
다음 교시는 통합 체크리스트, 평가 증거, 2~5주차 기술 매핑을 완성한다.
