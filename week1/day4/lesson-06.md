...
### Visual 1: 구조 다이어그램
![위험 분류와 Runbook 대응](./assets/lesson-06-risk-runbook.png)

이 이미지는 위험을 막연한 불안이 아니라 비용, 보안, 재현성, 운영 범주로 분류하게 한다. 각 범주는 README나 runbook에 남길 대응 행동과 evidence로 이어져야 한다.

```mermaid
flowchart TB
  Risk[Risk found] --> Cost[Cost]
  Risk --> Security[Security]
  Risk --> Repro[Reproducibility]
  Risk --> Ops[Operational]
  Cost --> Action[Response in README/runbook]
  Security --> Action
  Repro --> Action
  Ops --> Action
```
