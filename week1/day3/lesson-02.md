...
![Log, Config, Secret 경계](./assets/lesson-02-log-config-secret.png)

이 이미지는 log, config, secret을 모두 “텍스트”로 보지 않고 운영상 다른 보호 수준을 갖는 자료로 구분하게 한다. 강사는 secret 값 자체가 아니라 secret을 제외하는 판단 기준만 다룬다.

```mermaid
flowchart LR
    Run[Service 실행] --> Log[Log: 사건 기록]
    Run --> Config[Config: 실행 방식]
    Config --> Public[문서화 가능: key, non-sensitive value]
    Config --> Secret[Secret: token/password/key value]
    Secret --> Safe[값은 README와 log에서 제외]
    Log --> Evidence[status와 error message만 발췌]
```
