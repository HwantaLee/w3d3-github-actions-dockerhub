...
## 현업 사례: 장급 담당자 3명에게 묶였던 배포
한 조직에서는 배포 절차가 복잡했고, 인프라를 이해하는 사람이 적었다. 배포 담당자는 CTO와 개발팀장급 인원 3명이었다. 일반 개발자는 코드를 수정하고 repository에 올리는 것까지는 할 수 있었지만, container image를 Docker Hub나 registry에 올리고, EC2를 만들고, Auto Scaling Group에 반영하고, 배포 대상을 갱신하는 절차를 직접 수행하기 어려웠다.

문제는 담당자들이 모두 장급이라는 점이었다. 회의가 많고 의사결정 업무가 많아 배포 요청을 즉시 처리하기 어려웠다. 배포는 하루 한 번, 그것도 퇴근 시간 이후에 가능한 경우가 많았다. 개발자들은 기능 검증과 오류 수정을 하려면 배포를 기다려야 했고, 작은 수정도 다음 배포 시간을 기다리느라 밤 10시나 11시까지 남는 일이 반복되었다.

이 상황에서 병목은 개발자의 코드 작성 속도가 아니었다. 병목은 배포 권한과 절차가 소수 담당자에게 집중된 구조였다. 기능 개발은 끝났지만 개발환경이나 스테이징 환경에 빠르게 반영되지 못했고, 피드백이 늦어지면서 오류 수정도 늦어졌다. DORA 지표로 보면 deployment frequency는 낮고, lead time for changes는 길며, 장애나 오류를 발견했을 때 time to restore도 길어질 수밖에 없는 구조였다.

이후 DevOps Engineer가 합류해 GitHub Actions, AWS ECR, AWS ECS를 이용한 배포 파이프라인을 구성했다. 개발자가 GitHub에 push하면 workflow가 실행되고, container image를 ECR에 올리고, ECS 서비스가 새 버전을 배포하도록 단순화했다. 이 구조에서는 개발자가 배포 담당자에게 매번 요청하지 않아도 개발환경과 스테이징 환경에 변경사항을 반영할 수 있었다.

결과적으로 일별 배포 횟수는 50회 이상으로 늘었고, 배포 소요 시간은 약 1시간 30분에서 9분대로 줄었다. 이 변화는 단순히 "배포 명령이 빨라졌다"는 뜻이 아니다. 개발자가 기다리는 시간이 사라지고, 작은 단위로 자주 검증할 수 있게 되었으며, 오류를 발견하고 수정하는 피드백 루프가 짧아졌다는 뜻이다.

이 사례에서 배울 점은 다음과 같다.

| 이전 상태 | 자동화 이후 |
|---|---|
| 배포 담당자 3명에게 요청 필요 | GitHub push 또는 workflow로 배포 시작 |
| 하루 1회, 퇴근 후 배포 | 필요할 때 여러 번 배포 |
| 개발자는 배포 대기 때문에 야근 | 개발/스테이징 검증 대기 시간 감소 |
| Docker image/EC2/ASG 절차가 사람에게 의존 | ECR/ECS/GitHub Actions pipeline으로 표준화 |
| 실패 지점이 담당자 경험에 의존 | workflow log와 배포 이력으로 추적 |

초급자는 여기서 AWS 서비스 이름을 모두 외우려 할 필요는 없다. 중요한 구조는 "개발자가 코드를 push하면, 정해진 자동 절차가 build, image publish, deploy, health check를 수행한다"는 점이다. Docker Hub 대신 ECR을 쓸 수도 있고, EC2 Auto Scaling Group 대신 ECS를 쓸 수도 있으며, 이후에는 Kubernetes나 다른 배포 플랫폼을 사용할 수도 있다. 도구는 바뀌어도 병목을 자동화와 표준 절차로 푸는 원칙은 같다.
