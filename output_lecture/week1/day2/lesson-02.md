...
![GitHub, Git, VS Code 역할 구분](https://raw.githubusercontent.com/niceguy61/kdt_devops_lecture_2026_rev2/main/week1/day2/assets/lesson-02-git-vscode-roles.png)

이 이미지는 세 도구를 설치 목록이 아니라 협업 시스템의 서로 다른 책임으로 구분하게 한다. 학생에게 먼저 `clone`, `commit`, `push` 화살표를 읽게 한 뒤, 각 명령이 어느 경계를 넘는지 확인한다.

![Mermaid diagram 1](https://raw.githubusercontent.com/niceguy61/kdt_devops_lecture_2026_rev2/main/output_lecture/mermaid-assets/week1__day2__lesson-02--diagram-01.png)
...
처음 설치하는 학생은 공식 문서를 모두 읽으려 하기보다 설치 가이드의 순서대로 `git --version`, VS Code terminal의 `pwd`, `python3 --version`, `curl --version`을 먼저 확인한다. 실패하면 "어떤 도구가 안 되는지"를 분리해서 기록한다.
...
| 상황 | 진행 가능 여부 | 다음 행동 |
|---|---|---|
| `git --version` 성공, VS Code terminal에서 `pwd` 성공 | 진행 가능 | repository 실습으로 이동 |
| `code --version`만 실패 | 진행 가능 | VS Code GUI terminal을 사용하고 blocker에 기록 |
| `git --version` 실패 | 설치 확인 필요 | 설치 가이드의 Git 절차 수행 |
| GitHub 로그인은 되지만 push 실패 | 인증 확인 필요 | token 값 없이 에러 메시지만 기록 |
