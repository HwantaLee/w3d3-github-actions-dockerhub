# 4교시: queue/worker 운영 시나리오

## 수업 목표
- 작업 적체, worker 중지, 재처리 로그, 비동기 장애 확인를 실제 운영 판단과 연결한다.
- running, healthy, ready, user-visible success를 구분한다.
- 장애 리포트에 남길 증거를 고른다.

## 핵심 설명
worker는 사용자 요청을 직접 받지 않지만 API/queue 장애의 영향을 받는다.

## 실습 기준
```bash
cd week3/day1/labs/msa-demo
docker compose stop api && docker compose logs --tail=80 worker
```
