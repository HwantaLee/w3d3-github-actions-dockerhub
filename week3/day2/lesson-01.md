# 1교시: Day1 10분 요약 + 장애 전파와 부분 장애

## 수업 목표
- api/db/worker 장애가 사용자 경험에 미치는 영향를 실제 운영 판단과 연결한다.
- running, healthy, ready, user-visible success를 구분한다.
- 장애 리포트에 남길 증거를 고른다.

## 핵심 설명
Day1 baseline을 기준으로 API/DB/worker 장애가 어디까지 전파되는지 비교한다.

## 실습 기준
```bash
cd week3/day1/labs/msa-demo
docker compose ps && curl -s http://localhost:18084/health
```
