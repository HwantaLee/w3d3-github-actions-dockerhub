# 2교시: health check와 readiness

## 수업 목표
- `/health`, DB readiness, `depends_on`의 한계를 실제 운영 판단과 연결한다.
- running, healthy, ready, user-visible success를 구분한다.
- 장애 리포트에 남길 증거를 고른다.

## 핵심 설명
`/health`는 프로세스 생존이 아니라 dependency readiness까지 포함할 수 있다.

## 실습 기준
```bash
cd week3/day1/labs/msa-demo
docker compose stop db && curl -i http://localhost:18084/health
```
