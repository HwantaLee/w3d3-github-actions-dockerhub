...
## 수업 목표
- ImagePullBackOff, CrashLoopBackOff, Pending 상태를 describe/events/logs로 확인를 Kubernetes 실행 증거로 확인한다.
- manifest와 실제 cluster 상태를 비교한다.
- 실패 상태를 describe/events/logs로 읽는다.

## 핵심 설명
ImagePullBackOff, CrashLoopBackOff, Pending은 각각 확인할 증거가 다르다.
