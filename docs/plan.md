...
- 8교시 : Day3 미니 공유 - AI로 만든 작은 정적 웹사이트 초안, 실행 기록, 위험 제외, 컴퓨팅 spine 연결을 짧게 공유

## 4일차

- 1교시 : 쿠팡으로 보는 현대 애플리케이션 전체 지도 - 상품, 가격, 재고, 이미지, 주문, 서빙 계층
...
- 8교시 : 카카오페이로 보는 AI 플랫폼과 고성능 컴퓨팅 - GPU, Kubeflow, resource quota, 표준 실행 환경

## 5일차

- 1교시 : 애플리케이션 실행 환경은 코드만이 아니다 - runtime, dependency, program, port, config, data, command
...
- 8교시 : 비공식 미니 공유 - 만든 것, 확인한 것, 막힌 것, Docker 주차 질문을 짧게 공유

## 6일차

- 1교시 : 첫 1:1 멘토링 A그룹 - 학습 배경, 환경 상태, 학습 불안, blocker type 확인
...
- 4교시 : 작은 성공 경험 - local app 하나 실행 후 command, URL, status, stop step 기록
- 5교시 : Docker 준비도 triage - 설치 위험, 로컬 자원 확인, 예상 blocker note
- 6교시 : 스터디 그룹 라우팅 - blocker type과 속도에 따라 학생을 묶기
- 7교시 : 개인 회복 계획 - 다음 행동 3개와 검증 증거 정리
- 8교시 : Day6 closeout - Week2 Docker entry checklist
