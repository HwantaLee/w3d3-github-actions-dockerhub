...
- 개발 언어 제약은 없지만 브라우저에서 접속 가능한 웹 애플리케이션이어야 한다.
- GitHub 저장소에 코드와 README를 올린다.
- README에는 실행 방법, 사용한 AI 도구, 주요 프롬프트, 막힌 점, 해결 과정을 적는다.
- 로컬 브라우저에서 실행 가능해야 하며, 가능하면 Docker로도 실행할 수 있게 만든다.
- 최소 1개의 로그 또는 에러 상황을 설명할 수 있어야 한다.
...
- 4교시 : 미니 챌린지 진행 - AI와 함께 웹 애플리케이션 기능 완성, 로컬 실행 확인
