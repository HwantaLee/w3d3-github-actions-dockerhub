...
- 필수 개념은 제목, 표, 본문, 실습, 평가 증거에서 명확히 부각되어야 한다.
- 과밀 커리큘럼에서는 요약으로 통과할 수 없다. 수업 후 학생이 혼자 다시 읽고 따라올 수 있는 자습 복구 경로가 있어야 pass다.
- 작성자가 생각한 최소 필수 설명보다 200% 확장된 수준의 배경, 실패 모드, 판단 기준, 복구 절차, evidence가 있어야 pass다.
...
| essential_concept_emphasis | lesson 첫 부분에 `오늘 반드시 가져갈 것`이 있고, 필수 개념 3~5개가 왜 필수인지, 놓치면 어떤 문제가 생기는지, 어떤 evidence로 확인하는지 설명됨 |
| self_study_completeness | lesson 끝에 `혼자 다시 따라오기`가 있고, 최소 재현 경로, 공식 문서 키워드, 확인 명령/화면, 흔한 실패 3개, 제출 evidence, 다음 준비 상태가 있음 |
| depth_200_percent | 필수 개념이 단순 정의를 넘어 배경, 운영 문제, 한계, 실패 모드, 복구 절차, 판단 기준, 평가 증거까지 확장되어 있음 |
...
| 필수 개념 부각 | `오늘 반드시 가져갈 것`에서 필수 개념 3~5개를 우선순위로 제시하고, 놓치면 생기는 문제와 확인 증거를 연결함 |
...
| 자습 복구 | `혼자 다시 따라오기`에서 최소 성공 경로, 공식 문서 키워드, 흔한 실패와 첫 확인 위치를 제공함 |
...
- `1교시`, `Lesson 1`, `0-10분`, `50분 흐름`처럼 운영 시간표/교안 인덱스가 중심인 이미지
- 세션 목록이나 진행표를 그대로 그림으로 옮긴 이미지

다음 중 하나라도 해당하면 `fifty_minute_depth=0` 또는 `fail_rebuild_required`다.

- lesson이 1~2개 표와 짧은 bullet만 있고 설명 흐름이 없음
- lesson이 필수 개념을 우선순위로 부각하지 않고 모든 내용을 같은 비중으로 나열함
- lesson에 `혼자 다시 따라오기`가 없거나, 자습 복구 경로가 추상적 조언에 그침
- 필수 개념 설명이 정의 수준에 머물고 실패 모드, 복구, 판단 기준, evidence가 없음
...
- image asset이 시간표, 교시표, 세션 목차처럼 보여 학생용 개념 자료가 아니라 강사용 교안처럼 보임
